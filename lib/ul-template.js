...
async function read (api) {
  try {
    ...
    if (error?.statusCode == 404) {
      console.log(`\nThe ${api.tenantName} tenant currently has no html template set for the new universal login.`)
    } else if (error?.statusCode == 400) {
      console.log(`\nThe ${api.tenantName} tenant currently has no custom domain and therefore doesn't have a template set.`)
    } else {
